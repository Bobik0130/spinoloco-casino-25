...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-42549a4e";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-42549a4e">'+'<span class="tile-face-42549a4e tile-front-42549a4e"></span>'+'<span class="tile-face-42549a4e tile-back-42549a4e"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-42549a4e")){return;}
var back=tile.querySelector(".tile-back-42549a4e");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-42549a4e");if(isMine){tile.classList.add("boom-42549a4e");}
if(dim){tile.classList.add("dimmed-42549a4e");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-42549a4e")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-42549a4e");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-42549a4e")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-42549a4e");void panelEl.offsetWidth;panelEl.classList.add("jolt-42549a4e");}
endRound();}
